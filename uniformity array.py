...
(cloned from junction array.py)

Junction UNIFORMITY array: ebeam-only job for wafers whose optical layers
are ALREADY FABBED from junction array.py's masks. Measures junction-to-
junction critical-current scatter via room-temperature resistance
(Ambegaokar-Baratoff: dIc/Ic = dRn/Rn), following Kakuyanagi et al.,
APL 128, 232601 (2026): geometry terms separate by scaling, so a width
ladder across regions lets you fit them apart; nearest-neighbor
semivariograms need identical neighbors, so each region holds ONE size.

Differences from junction array.py:
  - width ladder on the TILE axis: 4 contiguous regions A-D (20x20 fields,
    400 junctions each), smallfinger 300/400/500/600 nm -- the paper's
    ladder (bigfinger locked to smallfinger + JJ_BIGFINGER_EXTRA)
  - NO dose sweeps: dose modulates junction size, which would inject the
    scatter being measured; all doses frozen at EBEAM_BASE_DOSES
  - the fabbed optical geometry (pads, tabs, shunts, TiW markers) is
    IDENTICAL to junction array.py -- do not change it here
  - FIELD_LABELS are ebeam-written in this job (they were NOT fabbed on
    the optical masks), so labels always match this sweep's mapping

PHYSICS TODO before fab: bridge width (0.84), undercut (0.2) and finger
length (1.5) were sized for ~140 nm fingers -- review for 300-650 nm.
"""
...
# Field labels are ebeam-written in this job (they were NOT fabbed on the
# optical masks), so the tiling is free to serve the statistics: 4 regions
# of 20x20 = 400 junctions per size, the paper's exact 300-600 nm ladder.
TILE_NX = 20
TILE_NY = 20

# Uniformity sweep: one junction size per contiguous region, nothing varies
# inside a region (nearest neighbors must be identical for the
# semivariogram), and no dose sweeps (dose modulates junction size).
SWEEP_SMALLFINGER_START = 0.300
SWEEP_COL = {}
SWEEP_ROW = {}
SWEEP_TILE = {                               # 4 regions A-D: 300 -> 600 nm
    'smallfinger_width': (SWEEP_SMALLFINGER_START, 0.100),
    'bigfinger_width':   (SWEEP_SMALLFINGER_START + JJ_BIGFINGER_EXTRA, 0.100),  # locked to smallfinger + JJ_BIGFINGER_EXTRA
}
...
EBEAM_BASE_DOSES = {
    ...
    'FIELD_LABELS': 600,  # per-field labels, ebeam-written in this job
}
...
def draw_field(chip, wafer, cx, cy, params, flabel):
    ...
    if DRAW_EBEAM_LAYERS:
        ...
        # bottom-left corner, clear of the pads and shunt. Ebeam-written in
        # this job (the fabbed optical masks carry no field labels), so the
        # labels always match this sweep's label -> parameter mapping.
...
LAYERS_EBEAM = [
    ...
    ['FIELD_LABELS', 7],   # per-field labels: ebeam-written in this job (NOT on the fabbed optical masks)
]
